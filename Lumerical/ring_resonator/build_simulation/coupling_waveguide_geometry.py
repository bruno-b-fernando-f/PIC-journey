# -*- coding: utf-8 -*-
"""
Created on Wed Dec  3 15:20:17 2025
...
and 'simulation_parameters.py' and render the slot waveguide structure below
...
The 2D slot waveguide is rendered on the 'y' and 'x' axis while the 'z' axis 
remains the propagation direction. The z-axis is not used during mode simulations.

The slot waveguide consists of two high-index rails separated by a narrow
low-index slot (gap). Strong field confinement occurs in the low-index slot region.
...
#%%
save_file = "\\ring_resonator_geometry.lms"
...
def waveguide_draw(mode):
    ...
    mode.addrect(name = "waveguide_1")
    mode.addrect(name = "waveguide_2")
    ...
    ("box",       (("x", 0.),
     ...
     ("waveguide_1", (("x", -gap_width/2-wg_width/2),
                    ("y min", 0),
                    ("z", 0.),
                    ("x span", wg_width),
                    ("y max", wg_thickness),
     ...
     ("waveguide_2", (("x", gap_width/2+wg_width/2),
                    ("y min", 0),
                    ("z", 0.),
                    ("x span", wg_width),
                    ("y max", wg_thickness),
