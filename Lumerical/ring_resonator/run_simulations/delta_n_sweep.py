# -*- coding: utf-8 -*-
"""
Created on Tue Dec  2 17:19:50 2025
based on scripts by gcharalampous https://github.com/gcharalampous/lumerical-py-scripts

@author: babreu
"""

"""
No user-inputs are required.

The scripts sweeps the width of the waveguide and calculates the effective
index for the number of modes you defined.
"""

#----------------------------------------------------------------------------
# Imports from user files
# ---------------------------------------------------------------------------

# Import necessary libraries and modules
import numpy as np
import lumapi
import os
import matplotlib.pyplot as plt
from config import *

#%%
# Import user-defined input parameters
from user_inputs.input_sweep_parameters import *    
from build_simulation.coupling_waveguide_geometry import waveguide_draw
from build_simulation.fde_region import add_fde_region  



# ------------------------- No inputs are required ---------------------------

#%%

def gapSweep(mode):
    
    # Create an array to store gap width values for each simulation
    gap_width_array = np.arange(gap_width_start, gap_width_stop, gap_width_step) 

    # Create empty arrays to store simulation results
    h, w = len(gap_width_array), num_modes
    neff = [[0 for y in range(w)] for x in range(h)] 
    polariz_frac = [[0 for y in range(w)] for x in range(h)] 
    polariz_mode = [[0 for y in range(w)] for x in range(h)] 

    # Loop over each gap width value and simulate the waveguide for each height
    for wd in range(0,len(gap_width_array)):
        mode.switchtolayout()    
        mode.setnamed("waveguide_1","x",-wg_width/2-gap_width_array[wd]/2)
        mode.setnamed("waveguide_2","x",wg_width/2+gap_width_array[wd]/2)
        mode.setnamed("mesh","x span",2*wg_width+gap_width_array[wd])
        mode.run()
        mode.mesh()
        mode.findmodes()
        # Turn on redraw feature to update simulation layout
        mode.redrawon()
        #mode.save(MODE_WAVEGUIDE_DIRECTORY_WRITE_FILE + "\\waveguide_mode_height_sweep_"+str(wd) + ".lms")

        
        for m in range(1,num_modes+1):
            # Get effective index and polarization fraction for each mode and store in corresponding 2D list
            neff[wd][m-1] = (mode.getdata("FDE::data::mode"+str(m),"neff"))
            polariz_frac[wd][m-1] = (mode.getdata("FDE::data::mode"+str(m),"TE polarization fraction"))
            
    #        if ( polariz_frac[m-1] > 0.5 ):   # identify the TE-like or TM-like modes
    #        	polariz_mode[wd][m-1] = ("TE")
    #        else:
    #            polariz_mode[wd][m-1] = ("TM")    
        
    # Squeeze the neff and TE polarization fraction arrays
    neff_array = np.squeeze(neff)
    polariz_frac_array = np.squeeze(polariz_frac)

    # Create a plot of neff vs. waveguide height for each mode
    plt.figure(1, figsize=(512/my_dpi, 256/my_dpi), dpi=my_dpi)
    for m in range(1,num_modes+1):
        plt.plot(gap_width_array*1e6,np.real(neff_array[:,m-1]),'-o', label = 'M-'+str(m))

    plt.legend()
    plt.grid(True, which='both')
    plt.xlabel("gap width (um)")
    plt.ylabel("neff")
    

    # Turn on redraw feature to update simulation layout
    mode.redrawon()

    # Save the figure files as .png
    file_name_plot = os.path.join(MODE_WAVEGUIDE_DIRECTORY_WRITE, "neff_gap_sweep.png")
    plt.savefig(file_name_plot, dpi=my_dpi, format="png")

    return neff_array, gap_width_array, polariz_frac_array, polariz_mode




if(__name__=="__main__"):
    with lumapi.MODE(hide = True) as mode:
        
        # Disable Rendering
        mode.redrawoff()

        # Draw the waveguide structure using a custom function
        waveguide_draw(mode)

        # Add a finite-difference eigenmode (FDE) region to the simulation environment
        add_fde_region(mode)

        # Plots effective index with widths
        
        neff_array, gap_width_array, polariz_frac_array, polariz_frac_mode = gapSweep(mode)
        

#%%
plt.close('all')

delta_n = (neff_array[:, 0] - neff_array[:, 1]).tolist()
        
plt.plot(gap_width_array*1e6, delta_n)
plt.xlabel(r"gap width ($\mu$m)")
plt.ylabel(r"$\Delta$n ($\mu$m)")
plt.grid()

#%%
file_name_data = os.path.join(MODE_WAVEGUIDE_DIRECTORY_WRITE_DATA, "delta_n_vs_gap_width.npy")
np.save(file_name_data, [gap_width_array, delta_n])
        
        
        
        
        
        
        
        
        
        
        
        